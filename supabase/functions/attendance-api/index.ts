...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (normalizedAction === 'create-attendance') {
      ...
      if (nowMinutes < 7 * 60 || nowMinutes > 21 * 60 + 30) {
        return json({ error: 'Attendance is only available between 07:00 and 21:30' }, 422);
      }
      const checkInTime = isOfflineSync ? String(normalizedPayload.check_in || '') : nowTime;
      const checkInMinutes = timeToMinutes(checkInTime);
      if (Number.isNaN(checkInMinutes) || checkInMinutes < 7 * 60 || checkInMinutes > 21 * 60 + 30) {
        return json({ error: 'Check-in time must be between 07:00 and 21:30' }, 422);
      }
    ...
    if (normalizedAction === 'update-attendance') {
      ...
      if (nowMinutes < 7 * 60 || nowMinutes > 21 * 60 + 30) return json({ error: 'Attendance is only available between 07:00 and 21:30' }, 422);
