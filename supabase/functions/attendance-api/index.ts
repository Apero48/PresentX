...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (normalizedAction === 'create-attendance') {
      ...
      if (nowMinutes < 7 * 60) {
        return json({ error: 'Attendance is only available from 07:00' }, 422);
      }
      const checkInTime = isOfflineSync ? String(normalizedPayload.check_in || '') : nowTime;
      const checkInMinutes = timeToMinutes(checkInTime);
      if (Number.isNaN(checkInMinutes) || checkInMinutes < 7 * 60) {
        return json({ error: 'Check-in time must be after 07:00' }, 422);
      }
